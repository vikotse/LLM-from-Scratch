        ...
        if isinstance(actual, dict):
            for key in actual:
                ...
                if actual[key] != expected_data[key]:
                    for word in actual[key]:
                        if word not in expected_data[key]:
                            print(f"Missing word in snapshot for key '{key}': {word}")
                    for word in expected_data[key]:
                        if word not in actual[key]:
                            print(f"Missing word in actual for key '{key}': {word}")
                    #for i, (v1, v2) in enumerate(zip(actual[key], expected_data[key])):
                    #    if v1 != v2:
                    #        print(f"Mismatch at index {i}: actual {v1}, expected {v2}")
